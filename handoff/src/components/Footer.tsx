  ...
  return (
    <footer className="mt-auto border-t border-slate-800 bg-slate-900">
      <div className="mx-auto flex max-w-6xl flex-col gap-8 px-6 py-12 md:flex-row md:items-start md:justify-between">
        ...
        <div className="max-w-xs">
          <Link href="/" className="text-lg font-bold tracking-tight">
            {settings.data.siteTitle ?? "Heirs of the Collapse"}
          </Link>
          {settings.data.tag ? (
            <p className="mt-2 text-sm text-on-ink-faint">{settings.data.tag}</p>
          ) : null}
        </div>
      ...
      <div className="border-t border-slate-800 px-6 py-4 text-center text-xs text-on-ink-faint">
        &copy; {new Date().getFullYear()} {settings.data.siteTitle ?? "Heirs of the Collapse"}. All rights reserved.
      </div>
    </footer>
  );
}
